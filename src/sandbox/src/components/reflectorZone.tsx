...
class Reflector {
  ...
  public constructor(engine: Engine, globalState: GlobalState) {
    ...
    this._webSocket = new WebSocket(
      `ws://${reflector.hostname}:${reflector.port}`
    );
    ...
    this._webSocket.onmessage = (event) => {
      ...
      if (StringTools.StartsWith(message, Reflector.SERVER_PREFIX)) {
        ...
        Logger.Log(
          `[Reflector] Received client message: ${message.substr(0, 64)}`
        );
  ...
  private _handleClientMessage(message: string): void {
    ...
    switch (command) {
      case "load": {
        SceneLoader.LoadAsync("", `data:${payload}`, this._engine).then(
          (scene) => {
            ...
            this._globalState.onSceneLoaded.notifyObservers({
              scene: scene,
              filename: "Reflector scene",
            });
