...
import { GLTFFileLoader, IGLTFLoaderExtension } from "@babylonjs/loaders";
...
export interface IRenderingZoneProps {
...
export class RenderingZone {
  // TODO extends React.Component<IRenderingZoneProps> {
  private _currentPluginName?: string;
  private _engine!: Engine;
  private _scene!: Scene;
  private _canvas!: HTMLCanvasElement;

  public constructor(readonly props: IRenderingZoneProps) {
    // TODO super(props);
  }
  ...
  async initEngine() {
    ...
    if (useWebGPU) {
      ...
      console.log("awaiting engine initAsync");
      await (this._engine as WebGPUEngine).initAsync();
      console.log(`engine done? ${this._engine._features}`);
    } else {
      ...
      console.log(`non webgpu engine created. ${this._engine._features}`);
    }
  ...
  loadAssetFromContents(fileName: string, contents: string) {
    this._engine.clearInternalTexturesCache();
    const promise = SceneLoader.LoadAsync("", `date:${contents}`, this._engine);
  ...
  loadAsset() {
    ...
    if (this.props.assetFileName && this.props.assetDoc) {
      this.loadAssetFromContents(this.props.assetFileName, this.props.assetDoc);
      return true;
    }
    return false;
  }
  ...
  componentDidMount() {
    ...
    // TODO
    (Animation as any).AllowMatricesInterpolation = true;
  ...
  public render() {
    return /* html */ `<div id="canvasZone" className="${
